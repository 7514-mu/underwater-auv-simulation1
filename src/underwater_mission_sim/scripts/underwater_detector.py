...
class UnderwaterDetector(Node):
    ...
    def _sonar_cb(self, msg):
        ...
        # 过滤范围
        valid_mask = (ranges > self.range_min) & (ranges < eff_max)
        ...
        object_mask = valid_ranges < (median_range - self.threshold)
        object_points = valid_points[object_mask]
        object_ranges = valid_ranges[object_mask]

        # 聚类
        clusters = self._cluster_points(object_points, object_ranges)
        ...
        self._publish_results(clusters)
