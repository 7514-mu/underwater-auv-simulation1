...
class ROV_NMB_SMController(DPControllerBase):
    ...
    def __init__(self, name, **kwargs):
        ...
        # Create separate dict for controller-specific services
        self._controller_services = dict()

        srv_name = 'set_sm_controller_params'
        self._controller_services[srv_name] = self.create_service(
    ...
    def update_controller(self):
        ...
        if self._update_count % 100 == 0:
            self._logger.info(f'update_controller called, count={self._update_count}')
        ...
        # Disable transient decay for better response
        sd_linear_b = np.zeros(3)  # Disabled: was np.multiply(self._s_linear_b_init, np.exp(-self._K[0:3] * (t - self._t_init)))
        sd_angular_b = np.zeros(3)  # Disabled: was np.multiply(self._s_angular_b_init, np.exp(-self._K[3:6] * (t - self._t_init)))
        ...
        if self._debug_count % 100 == 0:
            print(f'[DEBUG] Pos error={e_p_linear_b}, Vel error={e_v_linear_b}')
            print(f'[DEBUG] s_linear_b={s_linear_b}, sr_linear_b={sr_linear_b}')
            print(f'[DEBUG] force_b={force_b}')
