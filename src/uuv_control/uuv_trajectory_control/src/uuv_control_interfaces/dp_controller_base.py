...
class DPControllerBase(Node):
    ...
    * `reset_controller` (*service:* `uuv_control_msgs/ResetController`): Reset all 
    ...
    def __init__(self, name, is_model_based=False, list_odometry_callbacks=None,
        ...
        # Fix: Do not initialize _services as dict, let Node class use list
        # self._services = dict()  # BUG: dict doesn't have append()
        self._reset_service = self.create_service(ResetController,
                                                    'reset_controller',
    ...
    def _odometry_callback(self, msg):
        ...
        * `msg` (*type:* `nav_msgs/Odometry`): Input odometry
        ...
        if self._odom_callback_count % 100 == 0:
            self._logger.info(f'Odometry callback called, count={self._odom_callback_count}, callbacks={len(self._odometry_callbacks)}')
